...
class StaticSvgAdapter:
    name = "svg-resvg-sharp"
    ...
    def render(self, spec: dict[str, Any], output: Path) -> list[Path]:
        ...
        try:
            ...
            if payload and payload.get("outputs"):
                return [Path(p) for p in payload["outputs"]]
            if output.exists():
                return [output]
            raise RuntimeError("Static renderer returned no output files")
        finally:
