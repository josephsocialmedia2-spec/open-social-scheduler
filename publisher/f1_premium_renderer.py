...
def semantic_score(job: dict, item: dict, history: dict) -> int:
    ...
    need = job_need(job)
    score = 15 if "local" in kinds else 0
    score += 12 if "residential" in kinds else 0
    score += 8 if "apartment" in need and "apartment" in kinds else 0
    score += 7 if "house" in need and "house" in kinds else 0
    score += 8 if "townscape" in need and "townscape" in kinds else 0
...
def current_job(position: int) -> dict:
    ...
    for job in rows:
        if int(job.get("cycle_position", 0)) == int(position):
            return job
...
def fit_lines(draw: ImageDraw.ImageDraw, text: str, width: int, max_lines: int, start: int, minimum: int = 38):
...
def draw_brand(draw: ImageDraw.ImageDraw, x: int, y: int) -> None:
    draw.text((x, y), "F1", font=fnt(66, True, True), fill=WHITE)
    draw.line((x + 8, y + 7, x + 62, y - 16, x + 113, y + 8), fill=GREEN, width=5, joint="curve")
    draw.text((x + 118, y + 12), "IMMOBILIARE", font=fnt(24, True, True), fill=WHITE)
...
def qr_image(size: int = 100) -> Image.Image:
...
def prepare_photo(image: Image.Image, size: tuple[int, int]) -> Image.Image:
    ...
    return ImageEnhance.Color(photo).enhance(0.96)


def dark_gradient(size: tuple[int, int], horizontal: bool) -> Image.Image:
    w, h = size
...
def draw_headline(draw: ImageDraw.ImageDraw, text: str, x: int, y: int, width: int, max_lines: int, start: int) -> int:
    font, lines = fit_lines(draw, text, width, max_lines=max_lines, start=start)
    green_words = {"CASA", "VENDERE", "VENDITA", "VALORE", "DATI", "PREZZO", "F1", "TERRITORIO", "CRESCITA", "ESPERIENZA"}
    yy = y
    for line in lines:
        cursor = x
        for word in line.split():
            fill = GREEN if any(key in word for key in green_words) else WHITE
            draw.text((cursor, yy), word, font=font, fill=fill)
            cursor += draw.textbbox((0, 0), word + " ", font=font)[2]
        yy += int(getattr(font, "size", 54) * 1.03)
    return yy


def draw_bullets(draw: ImageDraw.ImageDraw, bullets: list[str], x: int, y: int) -> None:
    yy = y
...
def draw_cta(draw: ImageDraw.ImageDraw, text: str, x: int, y: int, width: int) -> None:
    draw.rounded_rectangle((x, y, x + width, y + 64), radius=18, fill=GREEN)
    font = fnt(21, True, True)
    label = text[:42]
    bbox = draw.textbbox((0, 0), label, font=font)
    tx = x + max(16, (width - (bbox[2] - bbox[0])) // 2)
...
def draw_footer(canvas: Image.Image) -> None:
    ...
    d.rectangle((0, y0, 1080, y0 + 8), fill=GREEN)
    draw_brand(d, 42, 1194)
    ...
    canvas.paste(qr_image(100), (930, 1200))
...
def variant_left_overlay(image: Image.Image, title: str, label: str, bullets: list[str], cta: str) -> Image.Image:
    ...
    canvas.alpha_composite(dark_gradient(SIZE, True))
    d = ImageDraw.Draw(canvas)
    ...
    y = draw_headline(d, title, 58, 250, 560, 5, 70)
...
def variant_photo_top(image: Image.Image, title: str, label: str, bullets: list[str], cta: str) -> Image.Image:
    ...
    shade.alpha_composite(dark_gradient((1080, 760), False))
    ...
    y = draw_headline(d, title, 58, 760, 920, 3, 64)
...
def variant_split(image: Image.Image, title: str, label: str, bullets: list[str], cta: str) -> Image.Image:
    canvas = Image.new("RGB", SIZE, BLACK)
    canvas.paste(prepare_photo(image, (650, 1165)), (430, 0))
    d = ImageDraw.Draw(canvas)
    ...
    y = draw_headline(d, title, 50, 265, 360, 6, 56)
    draw_bullets(d, bullets, 52, y + 25)
...
def variant_bottom_overlay(image: Image.Image, title: str, label: str, bullets: list[str], cta: str) -> Image.Image:
    ...
    draw_brand(d, 48, 38)
    d.text((58, 700), label, font=fnt(20, True, True), fill=GREEN)
    y = draw_headline(d, title, 58, 750, 900, 4, 68)
...
def premium_f1_composition(image: Image.Image, title: str, position: int) -> Image.Image:
    ...
    main_image = image
    if sid == "service_4_recruiting":
        presenter = "francesca" if str(job.get("role") or "") == "coordinatrice" else "joseph"
        try:
            main_image = renderer.load_presenter(presenter)
        except Exception:
            pass
...
def validate_f1_outputs() -> None:
    ...
    for job in rows:
        ...
        if dark_ratio < 0.55:
            raise RuntimeError(f"F1 premium footer gate failed for {rel}: dark={dark_ratio:.3f}")
