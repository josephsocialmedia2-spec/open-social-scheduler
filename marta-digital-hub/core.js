...
async function logout(){let old=S;clearSession();if(old?.access_token){try{await fetch(SUPA+'/auth/v1/logout',{method:'POST',headers:{apikey:KEY,Authorization:'Bearer '+old.access_token},keepalive:true})}catch{}}location.replace('./accesso.html?v=18')}
