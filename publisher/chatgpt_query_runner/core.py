...
def _new_job(row: dict[str, Any], query_index: int, sequence: int) -> dict[str, Any]:
    ...
    return {
        ...
        "search_intent": str(row.get("search_intent") or "").strip(),
        "audience": str(row.get("audience") or "").strip(),
        "variant": str(row.get("variant") or "").strip(),
        "concept_visual": str(row.get("concept_visual") or "").strip(),
        "provider": None,
        "provider_attempt": 0,
        "photorealism_score": None,
        ...
        "editorial_slot": str(row.get("editorial_slot") or "").strip(),
        "slot_key": str(row.get("slot_key") or "").strip(),
        "publication_status": str(row.get("publication_status") or "").strip(),
