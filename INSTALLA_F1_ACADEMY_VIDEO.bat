...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $repo='https://raw.githubusercontent.com/josephsocialmedia2-spec/open-social-scheduler/main/'; $lab='%LAB%'; $academy='%ACADEMY%'; $labFiles=@('INSTALLA_UGC_GRATIS.bat','AVVIA_ACADEMY_UGC.bat','ugc_server.py','ugc_training_server.py','ugc_tts.py','ARRESTA_UGC_GRATIS.bat'); foreach($f in $labFiles){Write-Host ('Scarico local-media-lab/'+$f); Invoke-WebRequest -UseBasicParsing -Uri ($repo+'local-media-lab/'+$f) -OutFile (Join-Path $lab $f)}; $academyFiles=@('index.html','styles.css','app.js','lessons.js'); foreach($f in $academyFiles){Write-Host ('Scarico f1-academy/'+$f); Invoke-WebRequest -UseBasicParsing -Uri ($repo+'f1-academy/'+$f) -OutFile (Join-Path $academy $f)}"
if errorlevel 1 (
